 ...
 * (`ideate-relay/internal/config.ValidateRelayOrigin`, with the same cases in its
 ...
 * operator from advertising a plaintext relay and finding out from their users.
...
/** The only port on which a plaintext relay origin is allowed.
 *
 ...
 *  for "a relay you run yourself" costs nothing and saves an explanation. */
export const LOCAL_RELAY_PORT = '7391'
 ...
 * on its own does not tell someone who pasted `https://relay.example.com/mcp`
 * that the problem is the path.
 */
export function validateRelayOrigin(raw: string): string | null {
  ...
  try {
    ...
    return 'That is not a URL. It should look like https://relay.example.com.'
  }
  ...
  if (url.pathname !== '/' && url.pathname !== '') {
    return 'Drop the path — this is just the service origin, like https://relay.example.com.'
  }
  ...
  if (url.protocol === 'http:') {
    ...
    if (local && url.port === LOCAL_RELAY_PORT) return null
    return (
      `Use https. Plain http is allowed only for a service you run yourself, on ` +
      `http://localhost:${LOCAL_RELAY_PORT}.`
    )
  }
...
export function normalizeRelayOrigin(raw: string): string {
 ...
 */
export function relayTabUrl(origin: string): string {
  const base = normalizeRelayOrigin(origin)
